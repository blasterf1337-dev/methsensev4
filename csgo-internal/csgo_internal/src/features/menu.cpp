...
//			const auto& cfg_list = config::list();
//
//			std::vector<std::string> names;
//			names.reserve(cfg_list.size());
//			for (auto& e : cfg_list)
...
//				selected = -1;
//
//			names.push_back(STR("+ New"));
//
//			int combo_val = selected >= 0 ? selected : (int)names.size() - 1;
//			if (ImGui::BeginCombo(STRC("##config_select"), names[combo_val].c_str())) {
//				for (int i = 0; i < (int)names.size(); ++i) {
//					if (ImGui::Selectable(names[i].c_str(), i == combo_val)) {
//						selected = i;
//					}
//				}
//				ImGui::EndCombo();
...
//		ImGui::Begin(dformat(STRS("weave | de0ver | Build: {} {}"), STR(__DATE__), STR(__TIME__)).c_str(), &open,
